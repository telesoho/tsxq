...
由于识别棋盘的API无法提供正确的当前是黑方还是红方回合，因此用户需要判断识别的棋盘是红方还是黑方的回合。因此做以下在棋盘的旁边上分别显示黑方和红方的两个计时器，分别表示黑方和红方的回合时间。
- 当通过API识别棋盘后，两个计时器都处于停止状态。
- 当黑方开始移动后，黑方计时器停止，红方计时器开始计时，表示轮到红方回合；
- 当红方开始移动后，红方计时器停止，黑方计时器开始计时，表示轮到黑方回合。
- 翻转棋盘后，计时器也应该翻转。
- 计时器显示在棋盘的左边垂直居中。
